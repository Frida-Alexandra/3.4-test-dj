...
def test_post_course_1(client):
    ...
    response = client.post(
        "/api/v1/courses/",
        data={"name": "course_1", "students": [student_1.id, student_2.id]},
    )
...
def test_post_course_2(client, student_factory):
    ...
    response = client.post(
        "/api/v1/courses/",
        data={"name": "course_1", "students": [i.id for i in students]},
    )
    assert response.status_code == 201
...
def test_patch_course(client, course_factory):
    ...
    response = client.patch(
        f"/api/v1/courses/{course[0].id}/", data={"students": [student.id]}
    )
    assert response.status_code == 200
    data = response.json()
